...
def identificar_salidas(mapa):
    ...
    while i < mapa.shape[0]:
        if mapa[i, mapa.shape[1] - 1] == 255 and i < mapa.shape[1]:
            ...
            nsalidas += 1
            salidas[nsalidas] = []
            while j < mapa.shape[1] and mapa[i, j] == 255:
                salidas[nsalidas].append([i, j])
                j += 1
            i = j
        else:
            i += 1
...
def jugador(mapa, area_navegable):
    ...
    while True:
        ...
        frame[y_start:y_end, x_start:x_end] = moving_image[
...
def verificar_coordenadas_en_area(coord, area_navegable):
    return coord in area_navegable
...
# Obtener área navegable
area_navegable_coords = [
    [i, j] for i in range(mapa.shape[0]) for j in range(mapa.shape[1]) if mapa[i, j] == 255
]

for mapa in mapas.values():
    mapa = cv2.resize(mapa, (600, 600), interpolation=cv2.INTER_NEAREST)
    jugador(mapa, area_navegable_coords)
